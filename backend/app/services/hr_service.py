...
def _require_employee(db:Session,employee_id:UUID,organization_id:UUID,lock:bool=False):
    q=db.query(Employee).filter(Employee.id==employee_id,Employee.organization_id==organization_id)
    employee=(q.with_for_update() if lock else q).first()
...
def create_employee(db,emp_in,organization_id,user_id):
    ...
    try:
        db.flush();log_action(db,organization_id,user_id,'CREATE','EMPLOYEE',emp.id,new_values=str({'user_id':str(emp_in.user_id),'employee_id':employee_code,'department':department,'designation':designation}));db.commit();db.refresh(emp);return emp
...
def create_salary_structure(db,struct_in,organization_id,user_id):
    _require_employee(db,struct_in.employee_id,organization_id,lock=True)
    ...
    try:
        db.flush();log_action(db,organization_id,user_id,'CREATE','SALARY_STRUCTURE',ss.id,new_values=str(struct_in.model_dump()));db.commit();db.refresh(ss);return ss
...
def get_payrolls(db,organization_id,employee_id=None,month=None,year=None):
...
def create_payroll(db,payroll_in,organization_id,user_id):
    _require_employee(db,payroll_in.employee_id,organization_id,lock=True);salary=db.query(SalaryStructure).filter(SalaryStructure.organization_id==organization_id,SalaryStructure.employee_id==payroll_in.employee_id).with_for_update().first()
    ...
    gross=salary.basic_salary+salary.hra
    if payroll_in.net_salary>gross:raise HTTPException(status_code=400,detail='Net salary cannot exceed configured gross salary')
    ...
    try:
        db.flush();log_action(db,organization_id,user_id,'CREATE','PAYROLL',py.id,new_values=str(payroll_in.model_dump()));db.commit();db.refresh(py);return py

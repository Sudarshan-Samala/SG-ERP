...
def get_fee_structures(db: Session, organization_id: UUID, grade_id: Optional[UUID] = None):
    ...
    if grade_id: query = query.filter(FeeStructure.grade_id == grade_id)
    return query.all()
...
def create_payment(db: Session, payment_in: PaymentCreate, organization_id: UUID, user_id: UUID):
    ...
    if paid + payment_in.amount_paid > invoice.amount_due:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Payment exceeds the outstanding invoice amount")

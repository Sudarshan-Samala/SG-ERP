...
def get_enquiries(db: Session, organization_id: UUID):
    return db.query(AdmissionEnquiry).filter(AdmissionEnquiry.organization_id == organization_id).order_by(AdmissionEnquiry.created_at.desc()).all()
...
def create_enquiry(db: Session, enquiry_in: AdmissionEnquiryCreate, organization_id: UUID):
    ...
    duplicate = db.query(AdmissionEnquiry).filter(
        ...
        AdmissionEnquiry.email == str(enquiry_in.email),
        AdmissionEnquiry.phone == enquiry_in.phone,

...
def create_session(db: Session, *, user_id, organization_id) -> IssuedSession:
    ...
    session = AuthSession(user_id=user_id, organization_id=organization_id, token_family_id=uuid.uuid4(), refresh_token_hash=hash_refresh_token(refresh_token), expires_at=datetime.utcnow() + timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS))
    db.add(session)
    db.commit()
    db.refresh(session)
...
def rotate_refresh_token(db: Session, *, session_id, organization_id, refresh_token: str) -> IssuedSession:
    session = db.query(AuthSession).filter(AuthSession.id == session_id, AuthSession.organization_id == organization_id).with_for_update().one_or_none()
    now = datetime.utcnow()
...
def revoke_session(db: Session, *, session_id, user_id, organization_id, reason: str = "logout") -> bool:
    session = db.query(AuthSession).filter(AuthSession.id == session_id, AuthSession.user_id == user_id, AuthSession.organization_id == organization_id).one_or_none()
    if session is None:
        return False
...
def revoke_all_sessions(db: Session, *, user_id, organization_id, reason: str = "logout_all") -> int:
    now = datetime.utcnow()
    sessions = db.query(AuthSession).filter(AuthSession.user_id == user_id, AuthSession.organization_id == organization_id, AuthSession.revoked_at.is_(None)).all()

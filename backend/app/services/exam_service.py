...
def get_exam_results(db, organization_id, exam_id=None, student_id=None):
    q=db.query(ExamResult).filter(ExamResult.organization_id==organization_id)
...
def create_exam_result(db, result_in: ExamResultCreate, organization_id, user_id):
    ...
    schedule=next((row for row in schedules if row.grade_id == getattr(student, "grade_id", None)), None) if getattr(student,"grade_id",None) else (schedules[0] if len(schedules)==1 else None)

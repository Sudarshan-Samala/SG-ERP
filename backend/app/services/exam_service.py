...
def create_exam_type(db, type_in, organization_id, user_id):
    ...
    et = ExamType(name=name, organization_id=organization_id)
...
def get_exams(db, organization_id, skip=0, limit=100):
    return db.query(Exam).filter(Exam.organization_id == organization_id).order_by(Exam.start_date.desc()).offset(skip).limit(limit).all()
...
def delete_exam(db, exam_id, organization_id, user_id):
    ...
    if db.query(ExamSchedule.id).filter(ExamSchedule.organization_id == organization_id, ExamSchedule.exam_id == exam_id).first():
        raise HTTPException(status_code=409, detail="Exam cannot be deleted while schedules exist")
    if db.query(ExamResult.id).filter(ExamResult.organization_id == organization_id, ExamResult.exam_id == exam_id).first():
        raise HTTPException(status_code=409, detail="Exam cannot be deleted while results exist")
    db.delete(exam); db.commit(); log_action(db, organization_id, user_id, "DELETE", "EXAM", exam_id)
    return True
...
def get_exam_schedules(db, organization_id, exam_id=None):
    ...
    return q.order_by(ExamSchedule.date).all()
...
def create_exam_schedule(db, schedule_in, organization_id, user_id):
    ...
    if duplicate:
        raise HTTPException(status_code=409, detail="Exam schedule already exists for this subject and grade")
    conflict = db.query(ExamSchedule).filter(ExamSchedule.organization_id == organization_id, ExamSchedule.exam_id == schedule_in.exam_id, ExamSchedule.grade_id == schedule_in.grade_id, ExamSchedule.date == schedule_in.date).first()
    if conflict:
        raise HTTPException(status_code=409, detail="This grade already has an exam scheduled at this date and time")
    obj = ExamSchedule(**schedule_in.model_dump(), organization_id=organization_id)
    db.add(obj); db.commit(); db.refresh(obj)
    log_action(db, organization_id, user_id, "CREATE", "EXAM_SCHEDULE", obj.id, new_values=str(schedule_in.model_dump()))
    return obj
...
def create_exam_result(db, result_in: ExamResultCreate, organization_id, user_id, *, commit: bool = True):
    exam = _require_owned(db, Exam, result_in.exam_id, organization_id, "Exam")
    ...
    if db.query(ExamResult).filter(ExamResult.organization_id == organization_id, ExamResult.exam_id == result_in.exam_id, ExamResult.student_id == result_in.student_id, ExamResult.subject_id == result_in.subject_id).first():
        raise HTTPException(status_code=409, detail="Exam result already exists")
    ...
    if schedule.date > exam.end_date:
        raise HTTPException(status_code=409, detail="Exam schedule is outside the configured exam window")

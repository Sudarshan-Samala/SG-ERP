...
def create_exam_type(db,type_in,organization_id,user_id):
    ...
    et=ExamType(name=name,organization_id=organization_id);db.add(et);db.commit();db.refresh(et);log_action(db,organization_id,user_id,"CREATE","EXAM_TYPE",et.id,new_values=str(type_in.model_dump()));return et
def get_exams(db,organization_id,skip=0,limit=100):return db.query(Exam).filter(Exam.organization_id==organization_id).order_by(Exam.start_date.desc()).offset(skip).limit(limit).all()
...
def get_exam_schedules(db,organization_id,exam_id=None):
    ...
    if exam_id:q=q.filter(ExamSchedule.exam_id==exam_id)
...
def create_exam_schedule(db,schedule_in,organization_id,user_id):
    ...
    if conflict:raise HTTPException(status_code=409,detail="This grade already has an exam scheduled at this date and time")
...
def create_exam_result(db,result_in:ExamResultCreate,organization_id,user_id):
    exam=_require_owned(db,Exam,result_in.exam_id,organization_id,"Exam");student=_require_owned(db,Student,result_in.student_id,organization_id,"Student");_require_owned(db,Subject,result_in.subject_id,organization_id,"Subject")

...
def get_finance_summary(db:Session,organization_id:UUID):
    rows=db.query(JournalEntry.type,func.coalesce(func.sum(JournalEntry.amount),0)).filter(JournalEntry.organization_id==organization_id).group_by(JournalEntry.type).all();totals={str(k).upper():float(v) for k,v in rows};debit=totals.get("DEBIT",0.0);credit=totals.get("CREDIT",0.0)
    return {"accounts":db.query(func.count(Account.id)).filter(Account.organization_id==organization_id).scalar() or 0,"entries":db.query(func.count(JournalEntry.id)).filter(JournalEntry.organization_id==organization_id).scalar() or 0,"total_debit":debit,"total_credit":credit,"difference":debit-credit}

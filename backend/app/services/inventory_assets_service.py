...
def _validate_name(value,field):
    cleaned=value.strip()
    if not cleaned:raise HTTPException(status_code=422,detail=f"{field} is required")
    return cleaned
def create_inventory_item(db,item_in,organization_id,user_id=None):
    data=item_in.model_dump();_validate_quantity(data);data["name"]=_validate_name(data["name"],"Inventory item name")
...
def update_inventory_item(db,item_id,item_in,organization_id,user_id=None):
    ...
    if "name" in data:
        data["name"]=_validate_name(data["name"],"Inventory item name")
...
def delete_inventory_item(db,item_id,organization_id):
    ...
    try:db.delete(item);db.commit();return True
    except Exception:db.rollback();raise
...
def create_asset(db,asset_in,organization_id,user_id=None):
    data=asset_in.model_dump();data["name"]=_validate_name(data["name"],"Asset name");data["asset_tag"]=_validate_name(data["asset_tag"],"Asset tag").upper()
...
def update_asset(db,asset_id,asset_in,organization_id,user_id=None):
    ...
    if "name" in data:data["name"]=_validate_name(data["name"],"Asset name")
    if "asset_tag" in data:data["asset_tag"]=_validate_name(data["asset_tag"],"Asset tag").upper()
...
def delete_asset(db,asset_id,organization_id):
    ...
    try:db.delete(asset);db.commit();return True
    except Exception:db.rollback();raise

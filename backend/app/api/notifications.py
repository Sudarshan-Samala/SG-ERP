...
class NotificationCreate(BaseModel):
 user_id:UUID;title:str=Field(min_length=2,max_length=160);message:str=Field(min_length=2,max_length=2000);category:str=Field(default='GENERAL',max_length=40);link:str|None=Field(default=None,max_length=500)
...
def list_notifications(unread_only:bool=False,limit:int=50,db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
 ...
 return [_serialize(x) for x in q.order_by(Notification.created_at.desc()).limit(min(max(limit,1),100)).all()]
@router.get('/summary')
...
def create_notification(payload:NotificationCreate,db:Session=Depends(get_db),current_user:User=Depends(require_permission('communication.manage'))):
 ...
 title=payload.title.strip();message=payload.message.strip();category=payload.category.strip().upper() or 'GENERAL';link=payload.link.strip() if payload.link else None

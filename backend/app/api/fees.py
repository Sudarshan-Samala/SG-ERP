...
from app.services.fee_service import get_fee_types, create_fee_type, get_fee_structures, create_fee_structure, get_invoices, create_invoice, get_payments, create_payment
...
def read_invoices(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), student_id: Optional[UUID] = None, _: User = Depends(require_permission("fees.read"))): return get_invoices(db, current_org.id, student_id)

@router.post("/invoices", response_model=Invoice)
def create_invoice_endpoint(invoice_in: InvoiceCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission("fees.invoice.create"))): return create_invoice(db, invoice_in, current_org.id, current_user.id)

@router.get("/payments", response_model=List[Payment])
def read_payments(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), invoice_id: Optional[UUID] = None, _: User = Depends(require_permission("fees.read"))): return get_payments(db, current_org.id, invoice_id)

@router.post("/payments", response_model=Payment)
def create_payment_endpoint(payment_in: PaymentCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission("fees.payment.collect"))): return create_payment(db, payment_in, current_org.id, current_user.id)

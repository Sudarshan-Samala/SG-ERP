...
from app.schemas.communication import Communication as CommunicationSchema,CommunicationCreate
from app.models.base import Communication as CommunicationModel,Organization,User,Branch,Grade
...
def set_target(communication_id:UUID,payload:TargetUpdate,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('communication.create'))):
 try:
  communication=db.query(CommunicationModel).filter(CommunicationModel.id==communication_id,CommunicationModel.organization_id==current_org.id).with_for_update().first()
...
@router.patch('/{communication_id}/status/{new_status}',response_model=CommunicationSchema)

...
def target_options(db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('communication.create'))):
 branch_ids=accessible_branch_ids(current_user);bq=db.query(Branch).filter(Branch.organization_id==current_org.id);gq=db.query(Grade).filter(Grade.organization_id==current_org.id)

...
@router.post('/signup',response_model=SessionToken,status_code=201)
def signup(payload:SignupRequest,request:Request,response:Response,db:Session=Depends(get_db)):
 signup_limiter.check(_rate_key(request,'signup'));ip,ua=_client_metadata(request);email=str(payload.email).strip().lower();org_name=' '.join(payload.organization_name.split());full_name=' '.join(payload.full_name.split())
...
def refresh(request_body:RefreshRequest,request:Request,response:Response,db:Session=Depends(get_db),refresh_token:str|None=Cookie(default=None,alias=settings.REFRESH_COOKIE_NAME)):
 refresh_limiter.check(_rate_key(request,'refresh'));require_csrf(request)

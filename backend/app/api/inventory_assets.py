...
def inventory_summary(db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),_:User=Depends(require_permission('inventory.read'))):
 items=get_inventory_items(db,current_org.id);assets=get_assets(db,current_org.id);return {'inventory_items':len(items),'stock_quantity':sum(i.quantity for i in items),'low_stock_items':sum(i.quantity<=5 for i in items),'out_of_stock_items':sum(i.quantity==0 for i in items),'assets':len(assets),'assets_in_repair':sum(a.status=='REPAIR' for a in assets),'disposed_assets':sum(a.status=='DISPOSED' for a in assets)}
...
def inventory_movements(item_id:UUID|None=None,limit:int=100,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),_:User=Depends(require_permission('inventory.read'))):
 return [{'id':x.id,'item_id':x.entity_id,'user_id':x.user_id,'previous':json.loads(x.previous_values or '{}'),'change':json.loads(x.new_values or '{}'),'created_at':x.created_at} for x in get_inventory_movements(db,current_org.id,item_id,limit)]

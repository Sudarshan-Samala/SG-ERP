...
def _strong(password: str) -> bool:
    return all((re.search(r'[a-z]', password), re.search(r'[A-Z]', password), re.search(r'\d', password), re.search(r'[^A-Za-z0-9]', password)))
...
def signup(payload: SignupRequest, request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    try:
        ...
        user = User(email=email, hashed_password=get_password_hash(payload.password), full_name=full_name, organization_id=organization.id, is_active=True, is_superuser=False, roles=[role])
    ...
    _set_refresh_cookie(response, issued.refresh_token); set_csrf_cookie(response, generate_csrf_token())
    record_auth_event(db, event_type='signup', outcome='success', organization_id=user.organization_id, user_id=user.id, session_id=issued.session.id, email=user.email, ip_address=ip, user_agent=request.headers.get('user-agent'))

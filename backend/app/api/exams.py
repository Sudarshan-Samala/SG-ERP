...
def read_results(db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),exam_id:Optional[UUID]=None,student_id:Optional[UUID]=None,branch_id:Optional[UUID]=None,current_user:User=Depends(require_permission('exam.result.read'))):
    allowed=None if current_user.is_superuser else accessible_branch_ids(current_user)
    if branch_id:
        enforce_branch_access(current_user,branch_id);allowed={branch_id}
    return get_exam_results(db,current_org.id,exam_id,student_id,allowed)
@router.get('/results/summary')
def result_summary(exam_id:UUID,branch_id:Optional[UUID]=None,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('exam.result.read'))):
    allowed=None if current_user.is_superuser else accessible_branch_ids(current_user)
    if branch_id:enforce_branch_access(current_user,branch_id);allowed={branch_id}
    rows=get_exam_results(db,current_org.id,exam_id,None,allowed);marks=[float(r.marks_obtained) for r in rows]
...
def bulk_results(payload:BulkResultRequest,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('exam.result.create'))):
    seen=set();student_ids={r.student_id for r in payload.results};students=db.query(Student).filter(Student.organization_id==current_org.id,Student.id.in_(student_ids)).all();by_id={s.id:s for s in students}
    if len(by_id)!=len(student_ids):raise HTTPException(status_code=400,detail='One or more students do not belong to this organization')
    for result in payload.results:
        ...
        seen.add(key);enforce_branch_access(current_user,by_id[result.student_id].branch_id)
    try:
        for result in payload.results:create_exam_result(db,result,current_org.id,current_user.id,commit=False)
        db.commit()

...
class BulkResultRequest(BaseModel):
    results: list[ExamResultCreate] = Field(min_length=1, max_length=200)


@router.get('/types', response_model=List[ExamType])
def read_exam_types(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), _: User = Depends(require_permission('exam.read'))):
    return get_exam_types(db, current_org.id)


@router.post('/types', response_model=ExamType)
def create_exam_type_endpoint(type_in: ExamTypeCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.manage'))):
    return create_exam_type(db, type_in, current_org.id, current_user.id)


@router.get('/', response_model=List[Exam])
def read_exams(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), skip: int = 0, limit: int = 100, _: User = Depends(require_permission('exam.read'))):
    return get_exams(db, current_org.id, skip, limit)


@router.post('/', response_model=Exam)
def create_exam_endpoint(exam_in: ExamCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.manage'))):
...
def read_schedules(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), exam_id: Optional[UUID] = None, _: User = Depends(require_permission('exam.read'))):
    return get_exam_schedules(db, current_org.id, exam_id)


@router.post('/schedules', response_model=ExamSchedule)
def create_schedule_endpoint(schedule_in: ExamScheduleCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.schedule.manage'))):
...
def read_results(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), exam_id: Optional[UUID] = None, student_id: Optional[UUID] = None, current_user: User = Depends(require_permission('exam.result.read'))):
    return get_exam_results(db, current_org.id, exam_id, student_id, None if current_user.is_superuser else accessible_branch_ids(current_user))
...
def result_summary(exam_id: UUID, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.result.read'))):
    rows = get_exam_results(db, current_org.id, exam_id, None, None if current_user.is_superuser else accessible_branch_ids(current_user))
    marks = [float(r.marks_obtained) for r in rows]
...
def bulk_results(payload: BulkResultRequest, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.result.create'))):
    seen = set()
    student_ids = {result.student_id for result in payload.results}
    students = db.query(Student).filter(Student.organization_id == current_org.id, Student.id.in_(student_ids)).all()
    student_by_id = {student.id: student for student in students}
    if len(student_by_id) != len(student_ids):
        raise HTTPException(status_code=400, detail='One or more students do not belong to this organization')
    for result in payload.results:
        key = (result.exam_id, result.student_id, result.subject_id)
        if key in seen:
            raise HTTPException(status_code=400, detail='Duplicate result in bulk request')
        seen.add(key)
        enforce_branch_access(current_user, student_by_id[result.student_id].branch_id)
    try:
        for result in payload.results:
            create_exam_result(db, result, current_org.id, current_user.id, commit=False)
        db.commit()
    except Exception:
        db.rollback()
        raise
    return {'created': len(payload.results)}
...
def create_result_endpoint(result_in: ExamResultCreate, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('exam.result.create'))):
    ...
    return create_exam_result(db, result_in, current_org.id, current_user.id)

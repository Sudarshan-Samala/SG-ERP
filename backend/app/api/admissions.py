...
class AdmissionConversion(BaseModel):
    admission_number:str=Field(min_length=1,max_length=50);date_of_birth:datetime;gender:Literal['male','female','other']
@router.get('/enquiries',response_model=List[AdmissionEnquiry])
def read_enquiries(db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('admissions.read'))):return get_enquiries(db,current_org.id,None if current_user.is_superuser else accessible_branch_ids(current_user))
@router.post('/enquiries',response_model=AdmissionEnquiry)
def create_enquiry_endpoint(enquiry_in:AdmissionEnquiryCreate,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('admissions.manage'))):enforce_branch_access(current_user,enquiry_in.branch_id);return create_enquiry(db,enquiry_in,current_org.id)
...
def convert_enquiry(enquiry_id:UUID,payload:AdmissionConversion,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('students.create'))):
    try:
        ...
        branch=db.query(Branch).filter(Branch.id==enquiry.branch_id,Branch.organization_id==current_org.id,Branch.is_active==True).first();year=db.query(AcademicYear).filter(AcademicYear.id==enquiry.academic_year_id,AcademicYear.organization_id==current_org.id).first()
        if not branch or not year:raise HTTPException(status_code=409,detail='Admission branch or academic year is no longer available')
        ...
        student=Student(organization_id=current_org.id,branch_id=enquiry.branch_id,academic_year_id=enquiry.academic_year_id,admission_number=number,student_name=enquiry.student_name.strip(),date_of_birth=payload.date_of_birth,gender=payload.gender,email=enquiry.email.strip().lower() if enquiry.email else None,phone=enquiry.phone.strip() if enquiry.phone else None);db.add(student);enquiry.status='ADMITTED';db.flush();log_action(db,current_org.id,current_user.id,'CONVERT','ADMISSION_ENQUIRY',enquiry.id,new_values=f'student_id={student.id}');db.commit();db.refresh(student);return {'student_id':student.id,'admission_number':student.admission_number,'student_name':student.student_name,'status':'ADMITTED'}

...
def create_enquiry_endpoint(enquiry_in:AdmissionEnquiryCreate,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('admissions.manage'))):
    enforce_branch_access(current_user,enquiry_in.branch_id);return create_enquiry(db,enquiry_in,current_org.id,current_user.id)
...
def change_enquiry_status(enquiry_id:UUID,new_status:str,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('admissions.manage'))):
    ...
    enforce_branch_access(current_user,enquiry.branch_id);return update_enquiry_status(db,enquiry_id,new_status,current_org.id,current_user.id)
...
def convert_enquiry(enquiry_id:UUID,payload:AdmissionConversion,db:Session=Depends(get_db),current_org:Organization=Depends(get_current_organization),current_user:User=Depends(require_permission('students.create'))):
    ...
    branch=db.query(Branch).filter(Branch.id==enquiry.branch_id,Branch.organization_id==current_org.id,Branch.is_active.is_(True)).first(); year=db.query(AcademicYear).filter(AcademicYear.id==enquiry.academic_year_id,AcademicYear.organization_id==current_org.id,AcademicYear.is_active.is_(True)).first()
    if not branch or not year:raise HTTPException(status_code=409,detail='Admission branch or academic year is no longer active')
    ...
    try:
        db.flush();log_action(db,current_org.id,current_user.id,'CONVERT','ADMISSION_ENQUIRY',enquiry.id,new_values=f'student_id={student.id}');db.commit();db.refresh(student)

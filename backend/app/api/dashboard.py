...
def get_dashboard_summary(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if current_user.is_superuser or "fees.read" in permissions:
        ...
        if branch_ids is not None:
            if branch_ids:
                invoice_query = invoice_query.filter(Student.branch_id.in_(branch_ids)); payment_query = payment_query.filter(Student.branch_id.in_(branch_ids))
            else:
                invoice_query = invoice_query.filter(False); payment_query = payment_query.filter(False)

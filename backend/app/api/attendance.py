...
def monthly_attendance(year: int, month: int, branch_id: Optional[UUID] = None, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('attendance.read'))):
    if year < 2000 or year > 2100 or month < 1 or month > 12:
        raise HTTPException(status_code=400, detail='Invalid attendance reporting period')
    if branch_id:
        enforce_branch_access(current_user, branch_id)
    start = datetime(year, month, 1)
    end = datetime(year + 1, 1, 1) if month == 12 else datetime(year, month + 1, 1)
    query = _attendance_query(db, current_org.id, current_user, branch_id).filter(AttendanceModel.date >= start, AttendanceModel.date < end)
    rows = query.all()
    statuses = Counter(str(row.status).upper() for row in rows)
    by_day = Counter(row.date.date().isoformat() for row in rows)
    marked = len(rows)
    present = statuses.get('PRESENT', 0)
    return {'year': year, 'month': month, 'records': marked, 'present': present, 'absent': statuses.get('ABSENT', 0), 'late': statuses.get('LATE', 0), 'present_rate': round((present / marked * 100) if marked else 0, 2), 'active_days': len(by_day), 'daily': [{'date': day, 'marked': count} for day, count in sorted(by_day.items())]}
...
def attendance_export(branch_id: Optional[UUID] = None, date: Optional[datetime] = None, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('attendance.read'))):
    ...
    def generate_csv():
        batch_size = 1000; offset = 0; first_batch = True
        while True:
            rows = query.offset(offset).limit(batch_size).all()
            if not rows:
                if first_batch: yield _csv_chunk([], {}, include_header=True)
                break
            ...
            yield _csv_chunk(rows, students, include_header=first_batch); first_batch = False; offset += len(rows)
...
def attendance_exceptions(date: datetime, branch_id: Optional[UUID] = None, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('attendance.read'))):
    ...
    student_rows = students.all(); marked_ids = {row.student_id for row in _attendance_query(db, current_org.id, current_user, branch_id, None, date).all()}
...
def bulk_attendance(payload: BulkAttendanceRequest, db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), current_user: User = Depends(require_permission('attendance.mark'))):
    ...
    if payload.date.date() > now.date(): raise HTTPException(status_code=400, detail='Attendance cannot be marked for a future date')
    seen = set(); parsed = []
    for item in payload.records:
        try: sid = UUID(str(item.get('student_id'))); status_value = str(item.get('status', '')).upper()
        except Exception as exc: raise HTTPException(status_code=400, detail='Invalid student attendance record') from exc
        ...
        parsed.append((sid, status_value))
    ...
    if len(students) != len(seen): raise HTTPException(status_code=400, detail='Student does not belong to the selected branch')
    ...
    if existing: raise HTTPException(status_code=409, detail='Attendance has already been marked for one or more students on this date.')
    for sid, status_value in parsed: db.add(AttendanceModel(branch_id=payload.branch_id, student_id=sid, date=payload.date, status=status_value, organization_id=current_org.id))
    try: db.commit()
    except IntegrityError as exc:
        db.rollback()
        if _constraint_name(exc) == 'uq_attendance_student_daily': raise HTTPException(status_code=409, detail='Attendance has already been marked for one or more students on this date.') from exc
        raise

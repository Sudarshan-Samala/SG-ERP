...
def read_attendance(db: Session = Depends(get_db), current_org: Organization = Depends(get_current_organization), branch_id: Optional[UUID] = None, student_id: Optional[UUID] = None, date: Optional[datetime] = None, skip: int = 0, limit: int = 100, _: User = Depends(require_permission("attendance.read"))):
    return get_attendance(db, organization_id=current_org.id, branch_id=branch_id, student_id=student_id, date=date, skip=skip, limit=min(max(limit, 1), 500))
